...
public:
    ...
    void processFrame(Vec3b* image_in, float exposure_time, float factor=1);
    void processFrame(unsigned char* image_in, float exposure_time, float factor=1);
    void unMapFloatImage(T2* image);

    ImageAndExposure<T1>* output;
...
public:
   ...
   ImageAndExposure<float>* undistort(const MinimalImageB* image_raw, float exposure=0, double timestamp=0, float factor=1) const;

    ImageAndExposure<Vec3f>* undistort3d(const MinimalImageB3* image_raw, float exposure=0, double timestamp=0, float factor=1) const;
	...
	void loadPhotometricCalibration(std::string file, std::string noiseImage, std::string vignetteImage);
     ...
     PhotometricUndistorter<T,unsigned char>* photometricUndist;
...
protected:
    ...
    void applyBlurNoise(Vec3f* img) const;
...
template class Undistort<float>;

 template<typename T>
class UndistortFOV : public Undistort<T>
{
public:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
 ...
 template<typename T>
class UndistortRadTan : public Undistort<T>
{
public:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
 ...
 template<typename T>
class UndistortEquidistant : public Undistort<T>
{
public:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
 ...
 template<typename T>
class UndistortPinhole : public Undistort<T>
{
public:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
...
private:
    float inputCalibration[9];
};

 template<typename T>
class UndistortKB : public Undistort<T>
{
public:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
...
template<typename T1, typename T2>
PhotometricUndistorter<T1,T2>::PhotometricUndistorter(
        std::string file,
        ...
        int w_, int h_)
{
    ...
    w = w_;
    h = h_;
    output = new ImageAndExposure<T1>(w,h);
...
template<typename T1, typename T2> PhotometricUndistorter<T1,T2>::~PhotometricUndistorter()
{
...
template<typename T1, typename T2> void PhotometricUndistorter<T1,T2>::unMapFloatImage(T2* image)
{
    int wh=w*h;
    if(sizeof(T1)/sizeof(float)<=1)
    {
...
template<typename T1, typename T2> void PhotometricUndistorter<T1,T2>:: processFrame(unsigned char* image_in, float exposure_time, float factor)
{
    int wh=w*h;
...
template<typename T1, typename T2> void PhotometricUndistorter<T1,T2>:: processFrame(Vec3b* image_in, float exposure_time, float factor)
{
    int wh=w*h;
    Vec3f* data = output->image;
...
Undistort<T>::~Undistort()
{
 ...
 template<typename T>
Undistort<T>* Undistort<T>::getUndistorterForFile(std::string configFilename, std::string gammaFilename, std::string vignetteFilename)
{
            ...
            &ic[4], &ic[5], &ic[6], &ic[7],&ic[8]) == 9)
    {
        ...
        u = new UndistortRadTan<T>(configFilename.c_str(), true);
            ...
            &ic[0], &ic[1], &ic[2], &ic[3], &ic[4]) == 5)
    {
        if(ic[4]==0)
        {
            ...
            u = new UndistortPinhole<T>(configFilename.c_str(), true);
            if(!u->isValid()) {delete u; return 0; }
        }
        else
        {
            printf("found ATAN camera model, building rectifier.\n");
            u = new UndistortFOV<T>(configFilename.c_str(), true);
            if(!u->isValid()) {delete u; return 0; }
        }
    }
    ...
    {
        u = new UndistortKB<T>(configFilename.c_str(), false);
        if(!u->isValid()) {delete u; return 0; }
    }
            ...
            &ic[4], &ic[5], &ic[6], &ic[7]) == 8)
    {
        u = new UndistortRadTan<T>(configFilename.c_str(), false);
        if(!u->isValid()) {delete u; return 0; }
    }
            ...
            &ic[4], &ic[5], &ic[6], &ic[7]) == 8)
    {
        u = new UndistortEquidistant<T>(configFilename.c_str(), false);
        if(!u->isValid()) {delete u; return 0; }
    }
            ...
            &ic[4]) == 5)
    {
        u = new UndistortFOV<T>(configFilename.c_str(), false);
        if(!u->isValid()) {delete u; return 0; }
    }
            ...
            &ic[4]) == 5)
    {
        u = new UndistortPinhole<T>(configFilename.c_str(), false);
        if(!u->isValid()) {delete u; return 0; }
    }
 ...
 template<typename T>
void Undistort<T>::loadPhotometricCalibration(std::string file, std::string noiseImage, std::string vignetteImage)
{
    photometricUndist = new PhotometricUndistorter<T,unsigned char>(file, noiseImage, vignetteImage,getOriginalSize()[0], getOriginalSize()[1]);
}

template<typename T>
ImageAndExposure<float>* Undistort<T>::undistort(const MinimalImageB* image_raw, float exposure, double timestamp, float factor) const
{
    ...
    ImageAndExposure<float>* result = new ImageAndExposure<float>(w, h, timestamp);
 ...
 template<typename T>
ImageAndExposure<Vec3f>* Undistort<T>::undistort3d(const MinimalImageB3* image_raw, float exposure, double timestamp, float factor) const
{
    if(image_raw->w != wOrg || image_raw->h != hOrg)
    {
        printf("Undistort::undistort: wrong image size (%d %d instead of %d %d) \n", image_raw->w, image_raw->h, w, h);
    ...
    //maybe, color image don't need photomentricUnidst
    //photometricUndist->processFrame<T>(image_raw->data, exposure, factor);
    ImageAndExposure<Vec3f>* result = new ImageAndExposure<Vec3f>(w, h, timestamp);
    //photometricUndist->output->copyMetaTo(*result);

    Vec3f* out_data = result->image;
    ...
    if (!passthrough)
    {
        ...
        for(int idx = w*h-1;idx>=0;idx--)
        {
            ...
            else
            {
                ...
                for(int i=0;i<3;++i)
                {
                out_data[idx][i] =  xxyy * src[1+wOrg][i]
                                    + (yy-xxyy) * src[wOrg][i]
                                    + (xx-xxyy) * src[1][i]
                                    + (1-xx-yy+xxyy) * src[0][i];
                }
            }
        }
    ...
    else
    {
        for(int idx = w*h-1;idx>=0;idx--)
        {
            result->image[idx][0]=in_data[idx][0];
            result->image[idx][1]=in_data[idx][1];
            result->image[idx][2]=in_data[idx][2];
        }
   }
 ...
 template<typename T>
void Undistort<T>::applyBlurNoise(float* img) const
{
 ...
 template<typename T>
void Undistort<T>::applyBlurNoise(Vec3f* img) const
{
    if(benchmark_varBlurNoise==0) return;
        ...
        for(int x=0;x<w;x++)
        {
            ...
            for(int ii=0;ii<3;++ii)
            {
                for(int dx=0; dx <= kernelSize; dx++)
                {
                    ...
                    {
                        sumW += gw;
                        sumCW[ii] += gw * img[x+dx+y*this->w][ii];
                    }
                ...
                blutTmp[x+y*this->w][ii] = sumCW[ii] / sumW;
            }
        }
        ...
        for(int y=0;y<h;y++)
        {
            ...
            for(int ii=0;ii<3;++ii)
            {
                for(int dy=0; dy <= kernelSize; dy++)
                {
                    ...
                    {
                        sumW += gw;
                        sumCW[ii] += gw * blutTmp[x+(y+dy)*this->w][ii];
                    }
                ...
                img[x+y*this->w][ii] = sumCW[ii] / sumW;
            }
        }
 ...
 template<typename T>
void Undistort<T>::makeOptimalK_crop()
{
 ...
 template<typename T>
void Undistort<T>::makeOptimalK_full()
{
    // todo
    assert(false);
}
...
template<typename T>
void Undistort<T>::readFromFile(const char* configFileName, int nPars, std::string prefix)
{
    photometricUndist=0;
    valid = false;
    passthrough=false;
...
template<typename T>
UndistortFOV<T>::UndistortFOV(const char* configFileName, bool noprefix)
{
    printf("Creating FOV undistorter\n");

    if(noprefix)
        this->readFromFile(configFileName, 5);
    else
        this->readFromFile(configFileName, 5, "FOV ");
}
 template<typename T>
UndistortFOV<T>::~UndistortFOV()
{
}

 template<typename T>
void UndistortFOV<T>::distortCoordinates(float* in_x, float* in_y, float* out_x, float* out_y, int n) const
{
    float dist = this->parsOrg[4];
    float d2t = 2.0f * tan(dist / 2.0f);



    // current camera parameters
    float fx = this->parsOrg[0];
    float fy = this->parsOrg[1];
    float cx = this->parsOrg[2];
    float cy = this->parsOrg[3];



    float ofx = this->K(0,0);
    float ofy = this->K(1,1);
    float ocx = this->K(0,2);
    float ocy = this->K(1,2);
 ...
 template<typename T>
UndistortRadTan<T>::UndistortRadTan(const char* configFileName, bool noprefix)
{
    printf("Creating RadTan undistorter\n");

    if(noprefix)
        this->readFromFile(configFileName, 9);
    else
       this-> readFromFile(configFileName, 9,"RadTan ");
}
 template<typename T>
UndistortRadTan<T>::~UndistortRadTan()
{
}

 template<typename T>
void UndistortRadTan<T>::distortCoordinates(float* in_x, float* in_y, float* out_x, float* out_y, int n) const
{
    // RADTAN
    float fx = this->parsOrg[0];
    float fy = this->parsOrg[1];
    float cx = this->parsOrg[2];
    float cy = this->parsOrg[3];
    float k1 = this->parsOrg[4];
    float k2 = this->parsOrg[5];
    float k3 = this->parsOrg[6];
    float r1 = this->parsOrg[7];
    float r2 = this->parsOrg[8];
 ...
 template<typename T>
UndistortEquidistant<T>::UndistortEquidistant(const char* configFileName, bool noprefix)
{
    printf("Creating Equidistant undistorter\n");

    if(noprefix)
        this->readFromFile(configFileName, 8);
    else
        this->readFromFile(configFileName, 8,"EquiDistant ");
}
 template<typename T>
UndistortEquidistant<T>::~UndistortEquidistant()
{
}

 template<typename T>
void UndistortEquidistant<T>::distortCoordinates(float* in_x, float* in_y, float* out_x, float* out_y, int n) const
{
    // EQUI
    float fx = this->parsOrg[0];
    float fy = this->parsOrg[1];
 ...
 template<typename T>
UndistortKB<T>::UndistortKB(const char* configFileName, bool noprefix)
{
    ...
    if(noprefix)
        ...
        this->readFromFile(configFileName, 8,"KannalaBrandt ");
}
 template<typename T>
UndistortKB<T>::~UndistortKB()
{
}

 template<typename T>
void UndistortKB<T>::distortCoordinates(float* in_x, float* in_y, float* out_x, float* out_y, int n) const
{
    const float fx = this->parsOrg[0];
    const float fy = this->parsOrg[1];
    const float cx = this->parsOrg[2];
    const float cy = this->parsOrg[3];
    const float k0 = this->parsOrg[4];
    const float k1 = this->parsOrg[5];
    const float k2 = this->parsOrg[6];
    const float k3 = this->parsOrg[7];

    const float ofx = this->K(0,0);
    const float ofy = this->K(1,1);
    const float ocx = this->K(0,2);
    const float ocy = this->K(1,2);
 ...
 template<typename T>
UndistortPinhole<T>::UndistortPinhole(const char* configFileName, bool noprefix)
{
    if(noprefix)
        this->readFromFile(configFileName, 5);
    else
        this->readFromFile(configFileName, 5,"Pinhole ");

}
 template<typename T>
UndistortPinhole<T>::~UndistortPinhole()
{
}

 template<typename T>
void UndistortPinhole<T>::distortCoordinates(float* in_x, float* in_y, float* out_x, float* out_y, int n) const
{
    // current camera parameters
    float fx = this->parsOrg[0];
    float fy = this->parsOrg[1];
