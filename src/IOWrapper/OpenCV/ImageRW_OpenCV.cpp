...
{
    cv::Mat m8u = cv::imread(filename, CV_LOAD_IMAGE_GRAYSCALE);
    cv::resize(m8u,m8u,cv::Size(1248,384));
    cv::Mat m,imgMatlF;
    m8u.convertTo(imgMatlF, CV_32F,1,1);
    cv::log(imgMatlF,imgMatlF);
////    归一化到0~255
    cv::normalize(imgMatlF, imgMatlF, 255, 0, CV_MINMAX);
    imgMatlF.convertTo(m, CV_8U,1,1);
	if(m.rows*m.cols==0)
	{
...
{
    cv::Mat m = cv::imdecode(cv::Mat(numBytes,1,CV_8UC3, data), CV_LOAD_IMAGE_COLOR);
    if(m.rows*m.cols==0)
    {
        printf("cv::imdecode could not read stream (%d bytes)! this may segfault. \n", numBytes);
        return 0;
