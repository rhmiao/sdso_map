...
PangolinDSOViewer::PangolinDSOViewer(int w, int h, std::string windowName,bool startRunThread)
{
	this->w = w;
	this->h = h;
...
void PangolinDSOViewer::run()
{
    ...
    pangolin::CreateWindowAndBind(windowName,2*w,2*h);
    ...
    pangolin::Var<bool> settings_showGroudTruth("ui.GroundTruth",true,true);
	...
	while( !pangolin::ShouldQuit() && running )
	{
		...
		if(setting_render_display3D)
		{
			...
			for(KeyFrameDisplay* fh : keyframes)
			{
                ...
                {
                    if(fh->record)fh->drawCam(1,yellow,0.05);
                    else fh->drawCam(1,blue,0.05);
                }
            ...
            currentCam->drawCamGroundTruth(2,green,0.1);
			drawConstraints();
			lk3d.unlock();
		}
        ...
        this->settings_showGroudTruth = settings_showGroudTruth.Get();
...
void PangolinDSOViewer::drawConstraints()
{
    ...
    if(settings_showGroudTruth)
    {
        ...
        for(unsigned int i=0;i<keyframes.size();i++)
        {
            glVertex3f((float)keyframes[i]->camToWorldGT.translation()[0],
                    (float)keyframes[i]->camToWorldGT.translation()[1],
                    (float)keyframes[i]->camToWorldGT.translation()[2]);
        }
        glEnd();
    }
...
void PangolinDSOViewer::publishRecordKeyframes(std::vector<std::vector<FrameHessian*>> &frames, FrameHessian* currentFrame,bool final, CalibHessian* HCalib)
{
    if(!setting_render_display3D) return;
    if(disableAllDisplay) return;
    ...
    int N=frames.size();
    for(int i=0;i<N;++i){
        for(FrameHessian* fh : frames[i])
        {
            int id = fh->recordID+fh->groupID*1000;
            if(keyframesByKFID.find(id) == keyframesByKFID.end())
            {
                KeyFrameDisplay* kfd = new KeyFrameDisplay();
                keyframesByKFID[id] = kfd;
                keyframes.push_back(kfd);
            }
            keyframesByKFID[id]->setFromKF(fh, HCalib);
            ...
            allFramePoses.push_back(fh->shell->camToWorld.translation().cast<float>());
        }
    }
    currentCam->setFromF(currentFrame->shell, HCalib);
...
void PangolinDSOViewer::pushLiveFrame(FrameHessian* image)
{
    ...
    for(int i=0;i<w*h;i++){
//        internalVideoImg->data[i][0] =image->imgdata.at<uchar>(i,0);
//        internalVideoImg->data[i][1] =image->imgdata.at<uchar>(i,1);
//        internalVideoImg->data[i][2] =image->imgdata.at<uchar>(i,2);
        internalVideoImg->data[i][0] =
        internalVideoImg->data[i][1] =
        internalVideoImg->data[i][2] =
            image->dI[i][0]*0.8 > 255.0f ? 255.0 : image->imgdata.at<uchar>(i,0)*0.8;
    }
