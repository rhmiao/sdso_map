...
void KeyFrameDisplay::drawCamGroundTruth(float lineWidth, float* color, float sizeFactor)
{
    if(width == 0)
        return;
    ...
    glPushMatrix();
        ...
        Sophus::Matrix4f m = camToWorldGT.matrix().cast<float>();
        glMultMatrixf((GLfloat*)m.data());
