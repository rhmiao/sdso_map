        ...
        const VecCf &xc, Mat18f* xAd, int min, int max, Vec10* stats, int tid)
{
	for(int k=min;k<max;k++)
	{
        ...
        if(!std::isfinite(p->data->step)){
            ...
            std::cout<<"p->data->step:"<<p->data->step<<std::endl;
            p->data->step=0;
            continue;
        }
...
//	std::cout << "EigPre:: " << eigenvaluesPre.transpose() << "\n";
	...
	std::vector<VecX> ns;
    ns.reserve(lastNullspaces_pose.size()+lastNullspaces_scale.size());
...
void EnergyFunctional::solveSystemOnestep(CalibHessian* HCalib)
{
    assert(EFDeltaValid);
    assert(EFAdjointsValid);
    assert(EFIndicesValid);
