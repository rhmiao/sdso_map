...
void CoarseTracker::makeCoarseDepthL0(std::vector<FrameHessian*> frameHessians1,std::vector<FrameHessian*> frameHessians2,CalibHessian* HCalib)
{
	// make coarse tracking templates for latstRef.
    memset(idepth[0], 0, sizeof(float)*w[0]*h[0]);
    memset(weightSums[0], 0, sizeof(float)*w[0]*h[0]);

    for(FrameHessian* fh : frameHessians1)
    {
		for(PointHessian* ph : fh->pointHessians)
		{
			if(ph->lastResiduals[0].first != 0 && ph->lastResiduals[0].second == ResState::IN)
			{
                ...
                if(r->host!=r->target)
                {
                    ...
                    if(u<0||v<0||u>w[0]||v>h[0]||new_idepth<0)continue;
    ...
    int start_frame = std::max((int)frameHessians2.size()-setting_recordKeyframesInLocalMap,0);
    int end_frame = frameHessians2.size();

    for(int i = start_frame;i<end_frame;++i)
    {
        FrameHessian* fh = frameHessians2[i];
        int flag=0;
        for(FrameHessian* fh2 : frameHessians1)
            if(fh2==fh){
                flag=1;
                break;
            }
        if(flag==1)continue;
//        if(!(fh->isUsedForPredictDepth))continue;
        for(PointHessian* ph : fh->pointHessians)
        {
            if(ph->lastResiduals[0].first != 0 && ph->lastResiduals[0].second == ResState::IN)
            {
        ...
        std::vector<FrameHessian*> frameHessians,std::vector<FrameHessian*> recordFrameHessians,CalibHessian* HCalib)
{
    ...
    makeCoarseDepthL0(frameHessians,recordFrameHessians,HCalib);
...
///
/// \brief CoarseDistanceMap::CoarseDistanceMap
/// \param ww
/// \param hh
///
...
////////////////////////////////////////
/// \brief CoarseTrackerWideAngle::CoarseTrackerWideAngle
/// \param ww
/// \param hh
///
CoarseTrackerWideAngle::CoarseTrackerWideAngle(int ww, int hh)
{
    //for better tracking

    // make coarse tracking templates.
    ww*=setting_panoramaTracker;
    hh*=setting_panoramaTracker;
    for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
    {
        int wl = ww>>lvl;
        int hl = hh>>lvl;
        ...
        idepth[lvl] = allocAligned<4,float>(wl*hl, ptrToDelete);
        color[lvl] = allocAligned<4,float>(wl*hl, ptrToDelete);
        weightSums[lvl] = allocAligned<4,float>(wl*hl, ptrToDelete);
        weightSums_bak[lvl] = allocAligned<4,float>(wl*hl, ptrToDelete);
...
CoarseTrackerWideAngle::~CoarseTrackerWideAngle()
{
}

void CoarseTrackerWideAngle::makeK(CalibHessian* HCalib)
{
    w[0] = setting_panoramaTracker*wG[0];
    h[0] = setting_panoramaTracker*hG[0];

    waddright=w[0]*(0.5f+0.5f/setting_panoramaTracker);
    haddbottom=h[0]*(0.5f+0.5f/setting_panoramaTracker);
    waddleft=w[0]*(0.5f-0.5f/setting_panoramaTracker);
    haddup=h[0]*(0.5f-0.5f/setting_panoramaTracker);

    fx[0] = HCalib->fxl();
    fy[0] = HCalib->fyl();
    cx[0] = HCalib->cxl()+wG[0]*float(setting_panoramaTracker-1)/2;
    cy[0] = HCalib->cyl()+hG[0]*float(setting_panoramaTracker-1)/2;
...
void CoarseTrackerWideAngle::makeCoarseDepthL0(std::vector<FrameHessian*> frameHessians1,std::vector<FrameHessian*> frameHessians2,CalibHessian* HCalib)
{
    // make coarse tracking templates for latstRef.
    memset(idepth[0], 0, sizeof(float)*w[0]*h[0]);
    memset(color[0], 0, sizeof(float)*w[0]*h[0]);
    memset(weightSums[0], 0, sizeof(float)*w[0]*h[0]);

    projectMaxX=0,projectMinX=w[0];
    projectMaxY=0,projectMinY=h[0];
    for(FrameHessian* fh : frameHessians1)
    {
        for(PointHessian* ph : fh->pointHessians)
        {
            if(ph->lastResiduals[0].first != 0 && ph->lastResiduals[0].second == ResState::IN)
            {
                ...
                {
                    assert(r->efResidual->isActive() && r->target == lastRef);
                    int u = r->centerProjectedTo[0] + waddleft;
                    int v = r->centerProjectedTo[1] + haddup;
                    float new_idepth = r->centerProjectedTo[2];
                    float weight = sqrtf(1e-3 / (ph->efPoint->HdiF+1e-12));
                    if(u<0||v<0||u>w[0]||v>=h[0]||new_idepth<0)continue;
                    idepth[0][u+w[0]*v] += new_idepth *weight;
                    color[0][u+w[0]*v]+=ph->color[0] *weight;
                    weightSums[0][u+w[0]*v] += weight;
                    if(projectMaxX<u)projectMaxX=u;
                    if(projectMinX>u)projectMinX=u;
                    if(projectMaxY<v)projectMaxY=v;
                    if(projectMinY>v)projectMinY=v;
                }
            }
        }
        ...
        {
            PointFrameResidual* r = new PointFrameResidual(ph, fh, lastRef);
            r->setState(ResState::IN);
            SE3 fhToNew = lastRef->PRE_worldToCam * (fh->PRE_camToWorld);
            Mat33f PRE_RTll_0 = (fhToNew.rotationMatrix()).cast<float>();
            Vec3f PRE_tTll_0 = (fhToNew.translation()).cast<float>();

            float drescale, uOld, vOld, new_idepth;
            float Ku, Kv;
            Vec3f KliP;

            projectPoint(ph->u, ph->v, ph->idepth_zero_scaled, 0, 0,HCalib,
                         PRE_RTll_0,PRE_tTll_0, drescale, uOld, vOld, Ku, Kv, KliP, new_idepth);

            int u = Ku + waddleft;
            int v = Kv + haddup;
            float weight = sqrtf(1e-3 *ph->idepth_hessian);
            if(u<0||v<0||u>w[0]||v>=h[0]||new_idepth<0)continue;
            idepth[0][u+w[0]*v] += new_idepth *weight;
            color[0][u+w[0]*v]+=ph->color[0] *weight;
            weightSums[0][u+w[0]*v] += weight;
    ...
    for(int i = start_frame;i<end_frame;++i)
    {
        ...
        if(flag==1)continue;
//        if(!(fh->isUsedForPredictDepth))continue;
        std::vector<PointHessian*> phVec(fh->pointHessians);
        phVec.reserve(fh->pointHessians.size()+fh->pointHessiansMarginalized.size());
        phVec.insert(phVec.end(),fh->pointHessiansMarginalized.begin(),fh->pointHessiansMarginalized.end()) ;
        for(PointHessian* ph : phVec)
        {
            PointFrameResidual* r = new PointFrameResidual(ph, fh, lastRef);
            r->setState(ResState::IN);
            SE3 fhToNew = lastRef->PRE_worldToCam * (fh->PRE_camToWorld);
    ...
    for(int lvl=1; lvl<pyrLevelsUsed; lvl++)
    {
        ...
        float* idepth_l = idepth[lvl];
        float* weightSums_l = weightSums[lvl];
        float* color_l = color[lvl];

        float* idepth_lm = idepth[lvlm1];
        float* weightSums_lm = weightSums[lvlm1];
        float* color_lm = color[lvlm1];
        ...
        for(int y=0;y<hl;y++)
            for(int x=0;x<wl;x++)
            {
                ...
                color_l[x + y*wl] = 		color_lm[bidx] +
                                            color_lm[bidx+1] +
                                            color_lm[bidx+wlm1] +
                                            color_lm[bidx+wlm1+1];
    ...
    for(int lvl=0; lvl<2; lvl++)
    {
        ...
        for(int it=0;it<numIts;it++)
        {
            ...
            float* idepthl = idepth[lvl];	// dotnt need to make a temp copy of depth, since I only
                                            // read values with weightSumsl>0, and write ones with weightSumsl<=0.
            float* colorl = color[lvl];
            for(int i=w[lvl];i<wh;i++)
            {
                if(weightSumsl_bak[i] <= 0)
                {
                    float sum=0, num=0, numn=0,sumcolor=0;
                    if(weightSumsl_bak[i+1+wl] > 0) { sumcolor+=colorl[i+1+wl];sum += idepthl[i+1+wl]; num+=weightSumsl_bak[i+1+wl]; numn++;}
                    if(weightSumsl_bak[i-1-wl] > 0) { sumcolor+=colorl[i-1-wl];sum += idepthl[i-1-wl]; num+=weightSumsl_bak[i-1-wl]; numn++;}
                    if(weightSumsl_bak[i+wl-1] > 0) { sumcolor+=colorl[i-1+wl];sum += idepthl[i+wl-1]; num+=weightSumsl_bak[i+wl-1]; numn++;}
                    if(weightSumsl_bak[i-wl+1] > 0) { sumcolor+=colorl[i+1-wl];sum += idepthl[i-wl+1]; num+=weightSumsl_bak[i-wl+1]; numn++;}
                    if(numn>0) {colorl[i]=sumcolor/numn;idepthl[i] = sum/numn; weightSumsl[i] = num/numn;}
                }
            }
        }
    }
    ...
    for(int lvl=2; lvl<pyrLevelsUsed; lvl++)
    {
        ...
        for(int i=w[lvl];i<wh;i++)
        {
            ...
            {
                float sum=0, num=0, numn=0,sumcolor=0;
                if(weightSumsl_bak[i+1] > 0) { sumcolor+=colorl[i+1];sum += idepthl[i+1]; num+=weightSumsl_bak[i+1]; numn++;}
                if(weightSumsl_bak[i-1] > 0) { sumcolor+=colorl[i-1];sum += idepthl[i-1]; num+=weightSumsl_bak[i-1]; numn++;}
                if(weightSumsl_bak[i+wl] > 0) { sumcolor+=colorl[i+wl];sum += idepthl[i+wl]; num+=weightSumsl_bak[i+wl]; numn++;}
                if(weightSumsl_bak[i-wl] > 0) { sumcolor+=colorl[i-wl];sum += idepthl[i-wl]; num+=weightSumsl_bak[i-wl]; numn++;}
                if(numn>0) {colorl[i]=sumcolor/numn;idepthl[i] = sum/numn; weightSumsl[i] = num/numn;}
            }
        }
    ...
    for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
    {
        ...
        for(int y=2;y<hl-2;y++)
            for(int x=2;x<wl-2;x++)
            {
                ...
                if(weightSumsl[i] > 0)
                {
                    int i_origin = (x-waddleft/scale)+(y-haddup/scale)*wG[0]/scale;
                    idepthl[i] /= weightSumsl[i];
                    colorl[i]/=weightSumsl[i];
                    lpc_u[lpc_n] = x;
                    lpc_v[lpc_n] = y;
                    lpc_idepth[lpc_n] = idepthl[i];
                    if(x<=waddleft/scale||y<=haddup/scale||x>=waddright/scale||y>=haddbottom/scale)
                        lpc_color[lpc_n]=colorl[i];
                    else
                        lpc_color[lpc_n] = dIRefl[i_origin][0];
...
Vec6 CoarseTrackerWideAngle::calcRes(int lvl, const SE3 &refToNew, AffLight aff_g2l, float cutoffTH)
{
    float E = 0;
    int numTermsInE = 0;
    int numTermsInWarped = 0;
    ...
    int wlNew = wG[lvl];
    int hlNew = hG[lvl];
    float fxlNew = fxG[lvl];
    float fylNew = fyG[lvl];
    float cxlNew = cxG[lvl];
    float cylNew = cyG[lvl];
    ...
    for(int i=0;i<nl;i++)
    {
        ...
        float u = pt[0] / pt[2];
        float v = pt[1] / pt[2];
        float Ku = fxlNew * u + cxlNew;
        float Kv = fylNew * v + cylNew;
        float new_idepth = id/pt[2];
        ...
        if(lvl==0 && i%32==0)
        {
            ...
            float uT = ptT[0] / ptT[2];
            float vT = ptT[1] / ptT[2];
            float KuT = fxlNew * uT + cxlNew;
            float KvT = fylNew * vT + cylNew;

            // translation only (negative)
            Vec3f ptT2 = Ki[lvl] * Vec3f(x, y, 1) - t*id;
            float uT2 = ptT2[0] / ptT2[2];
            float vT2 = ptT2[1] / ptT2[2];
            float KuT2 = fxlNew * uT2 + cxlNew;
            float KvT2 = fylNew * vT2 + cylNew;

            //translation and rotation (negative)
            Vec3f pt3 = RKi * Vec3f(x, y, 1) - t*id;
            float u3 = pt3[0] / pt3[2];
            float v3 = pt3[1] / pt3[2];
            float Ku3 = fxlNew * u3 + cxlNew;
            float Kv3 = fylNew * v3 + cylNew;

            //translation and rotation (positive)
            //already have it.

            float dx1=calcShift(KuT+waddleft-x,waddleft-projectMinX,projectMaxX-waddright);
            float dx2=calcShift(KuT2+waddleft-x,waddleft-projectMinX,projectMaxX-waddright);
            float dx3=calcShift(Ku+waddleft-x,waddleft-projectMinX,projectMaxX-waddright);
            float dx4=calcShift(Ku3+waddleft-x,waddleft-projectMinX,projectMaxX-waddright);

            float dy1=calcShift(KvT+haddup-y,haddup-projectMinY,projectMaxY-haddbottom);
            float dy2=calcShift(KvT2+haddup-y,haddup-projectMinY,projectMaxY-haddbottom);
            float dy3=calcShift(Kv+haddup-y,haddup-projectMinY,projectMaxY-haddbottom);
            float dy4=calcShift(Kv3+haddup-y,haddup-projectMinY,projectMaxY-haddbottom);

            sumSquaredShiftT += dx1*dx1+dy1*dy1;
            sumSquaredShiftT += dx2*dx2+dy2*dy2;
            sumSquaredShiftRT += dx3*dx3+dy3*dy3;
            sumSquaredShiftRT += dx4*dx4+dy4*dy4;

//            sumSquaredShiftT += (KuT+waddleft-x)*(KuT+waddleft-x) + (KvT+haddup-y)*(KvT+haddup-y);
//            sumSquaredShiftT += (KuT2+waddleft-x)*(KuT2+waddleft-x) + (KvT2+haddup-y)*(KvT2+haddup-y);
//            sumSquaredShiftRT += (Ku+waddleft-x)*(Ku+waddleft-x) + (Kv+haddup-y)*(Kv+haddup-y);
//            sumSquaredShiftRT += (Ku3+waddleft-x)*(Ku3+waddleft-x) + (Kv3+haddup-y)*(Kv3+haddup-y);
            sumSquaredShiftNum+=2;
        }

        if(!(Ku > 2 && Kv > 2 && Ku < wlNew-3 && Kv < hlNew-3 && new_idepth > 0)) continue;

        sumidepth+=new_idepth;

        float refColor = lpc_color[i];
        Vec3f hitColor = getInterpolatedElement33(dINewl, Ku, Kv, wlNew);
        if(!std::isfinite((float)hitColor[0])) continue;
        float residual = hitColor[0] - (float)(affLL[0] * refColor + affLL[1]);
        float hw = fabs(residual) < setting_huberTH ? 1 : setting_huberTH / fabs(residual);
...
float CoarseTrackerWideAngle::calcShift(float diff,float minDiff,float maxDiff)
{
    float shift=0;
//    if(minDiff<0)
        minDiff=0;
//    if(maxDiff<0)
        maxDiff=0;
    if(diff<0)shift=std::max(-diff,minDiff/2)-minDiff/2;
    else if(diff>0)shift=std::max(diff,maxDiff/2)-maxDiff/2;

    return shift;

}

bool CoarseTrackerWideAngle::trackNewestCoarse(
        FrameHessian* newFrameHessian,
        SE3 &lastToNew_out, AffLight &aff_g2l_out,
        int coarsestLvl,
        Vec5 minResForAbort,
        IOWrap::Output3DWrapper* wrap)
{
    ...
    newFrame = newFrameHessian;
    int maxIterations[] = {1,1,1,1,1};//{10,20,50,50,50};
    float lambdaExtrapolationLimit = 0.001;
...
void CoarseTrackerWideAngle::debugPlotIDepthMap(float* minID_pt, float* maxID_pt, std::vector<IOWrap::Output3DWrapper*> &wraps)
{
    if(w[1] == 0) return;
...

    {
        ...
        MinimalImageB3 mf(w[lvl], h[lvl]);
        MinimalImageB3 mfG(wG[lvl], hG[lvl]);
        mf.setBlack();
        mfG.setBlack();
        for(int i=0;i<hG[lvl];i++)
            for(int j=0;j<wG[lvl];j++)
            {
                int c = lastRef->dIp[lvl][i*wG[lvl]+j][0]*0.9f;
                if(c>255) c=255;
                mf.at((i+haddup)*w[lvl]+(j+waddleft)) = Vec3b(c,c,c);
            }
        int wl = w[lvl];
        for(int y=3;y<h[lvl]-3;y++)
            for(int x=3;x<wl-3;x++)
        ...
        cv::Mat mfGMat(hG[lvl],wG[lvl],CV_32FC3),mfMat(h[lvl],w[lvl],CV_32FC3);
        for(int i=0;i<h[lvl];i++)
            for(int j=0;j<w[lvl];j++)
            {
                mfMat.at<cv::Vec3f>(i,j)[0]=mf.at(j,i)[0];
                mfMat.at<cv::Vec3f>(i,j)[1]=mf.at(j,i)[1];
                mfMat.at<cv::Vec3f>(i,j)[2]=mf.at(j,i)[2];
            }
        cv::resize(mfMat,mfGMat,cv::Size(wG[lvl],hG[lvl]));
        for(int i=0;i<hG[lvl];i++)
            for(int j=0;j<wG[lvl];j++)
            {
                mfG.at(j,i)[0]=mfGMat.at<cv::Vec3f>(i,j)[0];
                mfG.at(j,i)[1]=mfGMat.at<cv::Vec3f>(i,j)[1];
                mfG.at(j,i)[2]=mfGMat.at<cv::Vec3f>(i,j)[2];
            }
//        IOWrap::displayImage("coarseDepth LVL0", &mf, false);

        for(IOWrap::Output3DWrapper* ow : wraps)
            ow->pushDepthImage(&mfG);
...
void CoarseTrackerWideAngle::debugPlotIDepthMapFloat(std::vector<IOWrap::Output3DWrapper*> &wraps)
{
    if(w[1] == 0) return;
    int lvl = 0;
    MinimalImageF mim(w[lvl], h[lvl], idepth[lvl]);
...
CoarseDistanceMapWideAngle::CoarseDistanceMapWideAngle(int ww, int hh)
{
    ww*=setting_panoramaTracker;
    hh*=setting_panoramaTracker;
    fwdWarpedIDDistFinal = new float[ww*hh/4];
...
CoarseDistanceMapWideAngle::~CoarseDistanceMapWideAngle()
{
}
...
void CoarseDistanceMapWideAngle::makeDistanceMap(
        std::vector<FrameHessian*> frameHessians,
        FrameHessian* frame)
{
    int w1 = w[1];
    ...
    for(FrameHessian* fh : frameHessians)
    {
        ...
        SE3 fhToNew = frame->PRE_worldToCam * fh->PRE_camToWorld;
        Mat33f KRKi = (K[1] * fhToNew.rotationMatrix().cast<float>() * KiG[0]);
        Vec3f Kt = (K[1] * fhToNew.translation().cast<float>());
        ...
        for(PointHessian* ph : fh->pointHessians)
        {
            assert(ph->status == PointHessian::ACTIVE);
            Vec3f ptp = KRKi * Vec3f(ph->u, ph->v, 1) + Kt*ph->idepth_scaled;
            int u = ptp[0] / ptp[2] + waddleft/2;
            int v = ptp[1] / ptp[2] + haddup/2;
            if(!(u > 0 && v > 0 && u < w[1] && v < h[1])) continue;
            fwdWarpedIDDistFinal[u+w1*v]=0;
            bfsList1[numItems] = Eigen::Vector2i(u,v);
            numItems++;
...
void CoarseDistanceMapWideAngle::makeInlierVotes(std::vector<FrameHessian*> frameHessians)
{

}


void CoarseDistanceMapWideAngle::addIntoDistFinal(int u, int v)
{
    if(w[0] == 0) return;
    bfsList1[0] = Eigen::Vector2i(u,v);
    fwdWarpedIDDistFinal[u+w[1]*v] = 0;
...
void CoarseDistanceMapWideAngle::makeK(CalibHessian* HCalib)
{
...
////////////////////////////////////////
/// \brief CoarseTrackerPanorama::CoarseTrackerPanorama
/// \param ww
/// \param hh
///
/*
CoarseTrackerPanorama::CoarseTrackerPanorama(int panoramaF)
{
    int ww=4*panoramaF;
    int hh=2*panoramaF;
    panoramaFocal[0]=panoramaF;
    for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
    {
        int wl = ww>>lvl;
        int hl = hh>>lvl;
...
CoarseTrackerPanorama::~CoarseTrackerPanorama()
{
    for(float* ptr : ptrToDelete)
        delete[] ptr;
    ptrToDelete.clear();
...
void CoarseTrackerPanorama::pinholeToPanorama(int pinhole_u,int pinhole_v,float depth,int &pano_u, int &pano_v,int level)
{
    float unit_f=1.0f;
    if(depth<0)unit_f=-1.0f;
    float unit_u=(pinhole_u-cx[level])/fx[level],unit_v=(pinhole_v-cy[level])/fy[level];
    pano_u=2*panoramaFocal[level]*(1+atan2(unit_u,unit_f)*M_1_PI)+0.5f;
    pano_v=panoramaFocal[level]*(1-M_2_PI*atan2(unit_v,sqrt(1+unit_v*unit_v)))+0.5f;
}

void CoarseTrackerPanorama::panoramaToPinhole(int pano_u, int pano_v,int &pinhole_u,int &pinhole_v,int level)
{
    float i_1_2pf=0.5f/panoramaFocal[level];
    if(pano_u>panoramaFocal[level]&&pano_u<3*panoramaFocal[level])
    {
        pinhole_u=tan((pano_u*i_1_2pf-1.0f)*M_PI)*fx[level]+cx[level]+0.5f;
        float unit_y_act2=tan((0.5f-pano_v*i_1_2pf)*M_PI);
        float unit_y2=unit_y_act2*unit_y_act2;
        float unit_y=sqrt(unit_y2/(1-unit_y2));
        pinhole_v=unit_y*fy[level]+cy[level]+0.5f;
    }
    else
    {
        pinhole_u=0;
...
void CoarseTrackerPanorama::makeK(CalibHessian* HCalib)
{
    w[0] = 4*panoramaFocal[0];
    h[0] = 2*panoramaFocal[0];
...
CoarseDistanceMapPanorama::CoarseDistanceMapPanorama(int ww, int hh)
{
    ww*=setting_panoramaTracker;
    hh*=setting_panoramaTracker;
    fwdWarpedIDDistFinal = new float[ww*hh/4];
...
CoarseDistanceMapPanorama::~CoarseDistanceMapPanorama()
{
    delete[] fwdWarpedIDDistFinal;
    delete[] bfsList1;
    delete[] bfsList2;
