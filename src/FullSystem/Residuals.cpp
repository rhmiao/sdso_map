...
//                state_NewState = ResState::OOB;
            ...
            hitcolorVec.push_back(hitColor);
        ...
        //getdeltaDis(hitcolorVec,color,hw4);
    ...
    {
        const Eigen::Vector3f* dIl = target->dIr[0];
        //const float* const Il = target->I;
        const Mat33f &PRE_KRKiTll = STEREO_PRE_KRKiTll;
        const Vec3f &PRE_KtTll = STEREO_PRE_KtTll;
        const Mat33f &PRE_RTll_0 = STEREO_PRE_RTll;
        const Vec3f &PRE_tTll_0 = STEREO_PRE_tTll;
        const float * const color = point->color;
        const float * const weights = point->weights;

        Vec2f affLL;//host->STEREO_PRE_aff_mode;
        affLL<<1,0;
        float b0 = 0;//host->STEREO_PRE_b0_mode;
        ...
        float wJI2_sum = 0;
        float stereo_optimize_lamda2=stereo_optimize_lamda*stereo_optimize_lamda;
        float stereo_lambda=3;//std::max(float(point->residuals.size()-1)/2,1.0f);
        float sqrt_stereo_lambda=stereo_lambda;//sqrt(stereo_lambda);
//        std::cout<<"point->numGoodResiduals:"<<std::max(float(point->residuals.size()-1),1.0f)<<" "<<stereo_lambda<<std::endl;
        for(int idx=0;idx<patternNum;idx++)
        {
            float Ku, Kv;
            if(!projectPoint(point->u+patternP[idx][0], point->v+patternP[idx][1], point->idepth_scaled, PRE_KRKiTll, PRE_KtTll, Ku, Kv))
            ...
            float hw = fabsf(residual) < setting_huberTH ? 1 : setting_huberTH / fabsf(residual);
            energyLeft += stereo_lambda*w*w*hw *residual*residual*(2-hw);
            hw = hw*sqrt_stereo_lambda;
            {
                if(hw < 1) hw = sqrtf(hw);
                hw = hw*w;
        ...
        if(energyLeft > stereo_lambda*std::max<float>(host->frameEnergyTH, target->frameEnergyTH) || wJI2_sum < stereo_lambda*2)
        {
            energyLeft = stereo_lambda*std::max<float>(host->frameEnergyTH, target->frameEnergyTH);
            state_NewState = ResState::OUTLIER;
        }
        else
        {
...
void PointFrameResidual::getdeltaDis(std::vector<Vec3f> hitcolorVec,const float * const color,float hw)
{
    int minIdx=-1;
    double mtmp,m=100;
    for(int i =0;i<hitcolorVec.size();++i){
        mtmp=fabs(hitcolorVec[i][0]-color[4]);
        if(mtmp<m)
        {
            minIdx = i;
            m= mtmp;
    ...
    deltaDis[0] = hw*patternP[minIdx][0];
    deltaDis[1] = hw*patternP[minIdx][1];
