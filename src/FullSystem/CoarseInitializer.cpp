...
int CoarseInitializer::trackFrame(FrameHessian* newFrameHessian, std::vector<IOWrap::Output3DWrapper*> &wraps)
{
...
//        }
    ...
    if(!snapped)
    {
        for(int lvl=0;lvl<pyrLevelsUsed;lvl++)
        {
            ...
            for(int i=0;i<npts;i++)
            {
                cv::Point2f pt(ptsl[i].u*scale,ptsl[i].v*scale);
                float dp=_imageMatch.depth.at<float>(int(ptsl[i].v*scale),int(ptsl[i].u*scale));
                if(dp>0.0001){
                    float idep=1/dp;
                    ptsl[i].iR = idep;
                    ptsl[i].idepth = idep;
                    ptsl[i].idepth_new = idep;
                    ptsl[i].lastHessian = 0;
                }
                else
                {
...
//                _imageMatch.getTriangleVetexInDelaunay(_imageMatch.subdiv, pt,triangle);
//                Vec3f target((pt.x-dso::cxG[0])*dso::fxiG[0],(pt.y-dso::cyG[0])*dso::fyiG[0],1.0f);
//                std::vector<Vec3f> vPt3;
//                if(triangle.size()==3)
//                {
//                    for(cv::Point2f pt2:triangle)
//                    {
//                        Vec3f pt3;
//                        pt3[2]=_imageMatch.depth.at<float>(int(pt2.y),int(pt2.x));
//                        pt3[0]=(pt2.x-dso::cxG[0])*dso::fxiG[0]*pt3[2];
//                        pt3[1]=(pt2.y-dso::cyG[0])*dso::fyiG[0]*pt3[2];
//                        vPt3.push_back(pt3);
//                    }
//                    float idep= 1.0f/_imageMatch.getInterpDepthInDelaunay(target,vPt3);
//                    if(std::isnan(idep)||idep<0.05f)idep=1;
//                    ptsl[i].iR = idep;
//                    ptsl[i].idepth = idep;
//                    ptsl[i].idepth_new = idep;
//                    ptsl[i].lastHessian = 0;
...
//                }
    ...
    int trackingFlag = 0;
//    SE3 refToNew_current=SE3();
    SE3 refToNew_current = _imageMatch.trackingWithOF(firstFrame,newFrame,w[0],h[0],0,KeyPointsRef, KeyPoints3d,matchRelAff,SE3(),trackingFlag);
    if(!trackingFlag)return -1;
    ...
    for(int lvl=pyrLevelsUsed-1; lvl>=0; lvl--)
    {
        ...
        resOld = calcResAndGS(lvl, H, b, Hsc, bsc, refToNew_current, refToNew_aff_current, false);
        applyStep(lvl);
        ...
        while(true)
        {
            ...
            SE3 refToNew_new = refToNew_current;//SE3::exp(inc.head<6>().cast<double>()) * refToNew_current;
            ...
            if(printDebug)
            {
                std::cout <<"accept:" <<"old:"<<eTotalOld<< " new:" << eTotalNew <<"\n";
        ...
        latestRes = resOld;
        //std::cout << fails<<"   "<<refToNew_current.log().transpose() << " AFF " << refToNew_aff_current.vec().transpose() <<"\n";
    }
    ...
    if(refToNew_current.translation().norm()<fabs(dso::bfG[0]*dso::fxiG[0]))return 0;
...
void CoarseInitializer::debugPlot(int lvl, std::vector<IOWrap::Output3DWrapper*> &wraps)
{
	...
	for(int i=0;i<wl*hl;i++)
        iRImg.at(i) = Vec3b((firstFrame->dI[i][0]),(firstFrame->dI[i][0]),(firstFrame->dI[i][0]));
        //iRImg.at(i) = Vec3b(firstFrame->imgdata.at<uchar>(i,0),firstFrame->imgdata.at<uchar>(i,0),firstFrame->imgdata.at<uchar>(i,0));
		...
		bool plot)
{
	...
	for(int i=0;i<npts;i++)
    {
		...
		for(int idx=0;idx<patternNum;idx++)
		{
            ...
            {//std::cout <<"Ku:" <<Ku<<"Kv"<< Kv << "wl"<<wl<< "hl"<<hl<<"\n";
				isGood = false;
				break;
			}
...
//
    ...
    float idpthscale;
    if(_imageMatch.trackingWithOFStereo(firstFrame,w[0],h[0],0,KeyPointsRef, KeyPoints3d,idpthscale)==0){
        frameID=-1;
    }
...
void CoarseInitializer::applyStep(int lvl)
{
	...
	for(int i=0;i<npts;i++)
	{
		if(!pts[i].isGood)
		{
            pts[i].idepth  = pts[i].iR = pts[i].idepth_new;
			continue;
		}
