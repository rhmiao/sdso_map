		...
		if(fixLinearization)
		{
			...
			if(r->efResidual->isActive())
			{
				if(r->isNew)
				{
                    ...
                    float relBS=0;
                    if(r->host==r->target){
                        Vec3f ptp_inf = STEREO_PRE_KRKiTll * Vec3f(p->u,p->v, 1);	// projected point assuming infinite depth.
                        Vec3f ptp = ptp_inf + STEREO_PRE_KtTll*p->idepth_scaled;	// projected point with real depth.
                        relBS = 0.01*((ptp_inf.head<2>() / ptp_inf[2])-(ptp.head<2>() / ptp[2])).norm();	// 0.01 = one pixel.
                    }
                    else
                    {
...
//	meanStepC += Hcalib.step.norm();
    ...
    float sumA=0, sumB=0,sumC=0, sumD=0, sumT=0, sumR=0, sumID=0, numID=0;
	...
	else
	{
		...
		for(FrameHessian* fh : frameHessians)
		{
            ...
            sumC += fh->step[8]*fh->step[8];
            sumD += fh->step[9]*fh->step[9];
    ...
    return sqrtf(sumA) < 0.0005*setting_thOptIterations &&
            sqrtf(sumB) < 0.00005*setting_thOptIterations &&
            sqrtf(sumC) < 0.0005*setting_thOptIterations &&
            sqrtf(sumD) < 0.00005*setting_thOptIterations &&
			sqrtf(sumR) < 0.00005*setting_thOptIterations &&
			sqrtf(sumT)*sumNID < 0.00005*setting_thOptIterations;
//
...
float FullSystem::OneStepOptimize(int mnumOptIts)
{
    ...
    LOG(INFO) << "The differences of doStepFromBackup: " << 1000.0*(end1 - start1)/cv::getTickFrequency()<<" ms";
    start1 = cv::getTickCount();
...
float FullSystem::optimize(int mnumOptIts)
{
    ...
    for(int iteration=0;iteration<mnumOptIts;iteration++)
	{
        ...
        end1 = cv::getTickCount();
        LOG(INFO) << "The differences of solveSystem: " << 1000.0*(end1 - start1)/cv::getTickFrequency()<<" ms";
        start1 = cv::getTickCount();
        ...
        end1 = cv::getTickCount();
        LOG(INFO) <<"The differences of calcMEnergy1: " << 1000.0*(end1 - start1)/cv::getTickFrequency()<<" ms";
        start1 = cv::getTickCount();
        double newEnergyL = calcLEnergy();
        double newEnergyM = calcMEnergy();
        ...
        end1 = cv::getTickCount();
        LOG(INFO) <<"The differences of loadSateBackup: " << 1000.0*(end1 - start1)/cv::getTickFrequency()<<" ms";
        start1 = cv::getTickCount();
    ...
    end1 = cv::getTickCount();
    LOG(INFO) <<"The differences of previousX: " << 1000.0*(end1 - start1)/cv::getTickFrequency()<<" ms";
...
void FullSystem::oneStepSolveSystem()
{
